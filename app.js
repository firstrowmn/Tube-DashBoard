...
function currentFiveMinuteMark(){
  return new Date(Math.floor(Date.now()/300000)*300000).toISOString();
}
...
function demoDevices(station, lineIndex){
  ...
  const measuredAt = currentFiveMinuteMark();
  return Array.from({length:count},(_,i)=>{
    const loss = ((base + i*13) % 42) / 1.3;
    return {deviceId:`DEMO-${station.id.slice(0,3).toUpperCase()}-${lineIndex+1}-${String(i+1).padStart(2,'0')}`, akp:`АКП ${i+1}`, ip:'—', lossPercent:+loss.toFixed(1), measuredAt, demo:true};
  });
}
...
function renderHome(){
  ...
  app.innerHTML=`<div class="summary"><div><h2>14 станций</h2><div class="muted">Нажмите на станцию, чтобы открыть линейки турникетов.</div></div><div class="muted">Замер — пятиминутный пинг, обновление примерно каждые 5 минут. Время отображается в GMT+7.</div></div><section class="grid">${cards}</section>`;
}
function renderStation(id){
  ...
  const panels=st.turnstileLines.map((line,li)=>{
    ...
    const rows=ds.map(d=>`<tr><td>${d.akp||d.deviceId}${d.demo?'<span class="demo-badge">ДЕМО</span>':''}</td><td>${d.ip||'—'}</td><td><span class="status-cell"><i class="dot ${thresholds(d.lossPercent)}"></i>${d.lossPercent.toFixed(1)}%</span></td><td>${formatActuality(d.measuredAt)}</td></tr>`).join('');
    return `<section class="line-panel"><h3>${line.name}</h3>${rows?`<table><thead><tr><th>№ турникета / АКП</th><th>IP</th><th>Потери связи</th><th>Актуально на</th></tr></thead><tbody>${rows}</tbody></table>`:'<div class="empty">Нет оборудования в справочнике.</div>'}</section>`
  }).join('');
